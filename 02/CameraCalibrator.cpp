...
void CameraCalibrator::setBoardSize(QString bx,QString by,QString sx,QString sy){
     board_size = Size(bx.toInt(),by.toInt());   /* 标定板上每行、列的角点数 */
     square_size = Size(sx.toInt(),sy.toInt());  /* 实际测量得到的标定板上每个棋盘格的大小 */

}
vector<double> CameraCalibrator::getCameraMatrix(){
    return cameraMatrix_vec;
}
        ...
        for(int ncol = 0; ncol < 3; ncol++)
            {
                ...
                cameraMatrix_vec.push_back(cameraMatrix.at<double>(nrow,ncol));
               cout<<cameraMatrix_vec[nrow*3+ncol]<<endl;

            }
        //打印畸变矩阵每一个参数
        for( int ncol = 0; ncol < 5; ncol++)
        {
            distCoeffs_vec.push_back(distCoeffs.at<double>(0,ncol));
            cout<<distCoeffs_vec[ncol]<<endl;
        }
